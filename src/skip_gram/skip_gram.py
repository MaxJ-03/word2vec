...
class SkipGram(Word2VecBase):
    ...
    def backpropagation(self, input_vector_id, context_vectors_ids, context_size):
        ...
        EI = context_size * self.y_pred # EI_j = C * y_j - sum(t_{c,j}) (Eq. 34)
        ...
        self.W2 -= self.learning_rate * np.outer(self.h, EI) # v'_w_j(new) = v'_w_j(old) - learning_rate * EI_j * h (Eq. 37)

        # Update the input-to-hidden weight specifically for the central input word used in the current sample.
        self.W1[input_vector_id] -= self.learning_rate * EH # v_w_I(new) = v_w_I(old) - learning_rate * dE/dh (Eq. 38)
    ...
    def train(self, epochs, print_interval=100000):
        ...
        for epoch in range(epochs):
            ...
            for i in range(self.data_processing.data_length):
                ...
                epoch_loss += -np.sum(np.log(predicted_vector[context_vectors_ids] + 1e-9)) # (Eq. 26)
